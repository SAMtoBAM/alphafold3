...
if [[ -n "$SINGIMG" ]] ; then
  ...
  apptainer exec \
    ...
    --cwd /app/AF3Complex \
    ${SINGIMG} \
    TMPDIR=/root/tmp python run_feature_generation.py \
    --db_dir=/root/public_databases \
    --json_file_path=/root/af_input/fold_input_server.json \
    --model_dir=/root/models \
    --feature_dir=/root/af_output \
  ...
  TMPDIR="${WORK_TMP_DIR}" python run_feature_generation.py \
  --json_file_path="${WORK_DIR_FULL_PATH}/af_input/fold_input_server.json" \
  --model_dir="${WORK_DIR_FULL_PATH}/models" \
  --db_dir="${EXTRACTED_DATABASE_FULL_PATH}" \
  --feature_dir="${WORK_DIR_FULL_PATH}/af_output" \
  --input_json_type='server'
