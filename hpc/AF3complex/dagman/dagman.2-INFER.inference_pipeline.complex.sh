...
VERBOSE_LEVEL=2 # 0 = silent, 1 = info, 2 = verbose
...
if compgen -G "*.data_pipeline.tar.gz"  > /dev/null; then
  ...
  do
    # get base name without the .tar.gz extension
    subdir_name=$(basename "${filename}" .tar.gz)
    # make subdirectory inside WORK_INPUT_DIR
    mkdir -p "${WORK_INPUT_DIR}/${subdir_name}"
   printverbose "Extracting to WORK_INPUT_DIR : ${filename}"
   tar zxf "${filename}" -C ${WORK_INPUT_DIR}/${subdir_name}/
...
if [[ -n "$SINGIMG" ]] ; then # use apptainer to run the container
  apptainer exec \
     ...
     --feature_dir_path=/root/af_input \
     --model_dir=/root/models \
     --output_dir=/root/af_output \
     $EXTRA_RUN_ALPHAFOLD_FLAGS   
else # we must already be in the container
  WORK_DIR_FULL_PATH=`realpath ${WORK_DIR}` # full path to working directory
  printinfo "Attemping to run run_af3complex_inference.py"
  pushd /app/AF3Complex
  python run_af3complex_inference.py --help
  python run_af3complex_inference.py \
       --db_dir="${WORK_DIR_FULL_PATH}/public_databases" \
       --model_dir="${MODEL_PARAM_DIR}" \
       --feature_dir_path="${WORK_DIR_FULL_PATH}/af_input" \
...
do
  ...
  ##remove some files I'll never use and reduce size of everything
  rm -r ${output_dir}/seed*
  rm ${output_dir}/*_data.json
  rm ${output_dir}/ranking_scores.csv
